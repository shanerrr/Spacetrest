...
const PictureListItem = ({ photo, lastGalleryItem, likes, onLikeClick, setModalDetails }: { photo: image, lastGalleryItem: (node: HTMLElement | null) => void, likes: { [key: string]: boolean }, onLikeClick: (url: string, event: React.MouseEvent<HTMLElement>) => void, setModalDetails: (photo: image) => void }) => {
  ...
  return (
    <article ref={lastGalleryItem} className={`${photo.colLength === 1 ? 'col-span-1' : 'col-span-1 md:col-span-2'} bg-white rounded-[10px] p-4 animate-fadeInBottom cursor-pointer`} onClick={() => setModalDetails(photo)}>
      <div className='relative h-80'>
        <Image placeholder="blur" alt={photo.date+ ' APOD'} className='rounded-[10px]' src={photo.title} loader={() => photo.url} layout='fill' objectFit='cover' blurDataURL="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkMAYAADkANVKH3ScAAAAASUVORK5CYII=" />
