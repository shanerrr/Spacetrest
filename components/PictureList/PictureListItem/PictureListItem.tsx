...
const PictureListItem = ({ photo, likes, onLikeClick, setModalDetails }: { photo: IState['photoDetails'], likes: { [key: string]: boolean }, onLikeClick: (url: string, event: React.MouseEvent<HTMLElement>) => void, setModalDetails: (photo: IState['photoDetails']) => void }) => {
  ...
  return (
    <article key={photo.hdurl} className={`${photo.colLength === 1 ? 'col-span-1' : ''} ${photo.colLength === 2 ? 'col-span-2' : ''} bg-white shadow-md hover:shadow-2xl hover:transition-shadow rounded-[10px] p-4 animate-fadeInBottom cursor-pointer`} onClick={() => setModalDetails(photo)}>
