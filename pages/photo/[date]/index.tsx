...
const PhotoOfDate = ({ imageResponse }: { imageResponse: image }) => {
  ...
  const likeHandler = (url: string, event: React.MouseEvent<HTMLElement>) => {
    ...
    localStorage.setItem(
      "likes",
      JSON.stringify({ ...likes, [url]: !!!likes[url] })
    );
  };
  ...
  React.useEffect(
    () =>
      setLikes(
        localStorage.hasOwnProperty("likes")
  ...
  return (
    <>
      <Head>
        <title>Spacetrest: Picture of the day</title>
      ...
      <main className="snap-y snap-mandatory h-max bg-[#181A18]">
        ...
        <section className="h-screen relative align-middle snap-center">
          <PictureOfDay
            ...
            isLiked={likes[imageResponse.hdurl]}
            onLikeClick={(url: string, event: React.MouseEvent<HTMLElement>) =>
              likeHandler(url, event)
            }
            scrollToGallery={undefined}
          />
        </section>
      </main>
    </>
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  if (ctx.params?.date) {
    return {
      props: {
        imageResponse: {
          ...(await new ImageService().getSpecificImageOfDay(ctx.params.date))
            .data,
          show: true,
        },
      },
